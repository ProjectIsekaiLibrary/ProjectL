...
KunrealEngine::Kamen::Kamen()
	...
	_swordRotationAttack(nullptr), _swordMultipleAttack(nullptr)
{
	BossBasicInfo info;
...
void KunrealEngine::Kamen::CreateParticleObject()
{
	...
	}
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				donutParticle3->GetComponent<Particle>()->SetParticleSize(120.f, 120.0f);
			}
			else
			{
...
void KunrealEngine::Kamen::CreateSubObject()
{
	...
	// 카멘에 종속된 검
	{
		...
		_kamenSword->SetTotalComponentState(true);
...
void KunrealEngine::Kamen::CreateSwordLinearAttack()
{
	...
	pattern->SetAnimName("Idle").SetSpeed(60.0f).SetDamage(10.0f);
	pattern->SetAttackState(BossPattern::eAttackState::ePush);
