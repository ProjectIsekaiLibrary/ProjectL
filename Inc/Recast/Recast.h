...
/// Recast에서 사용하는 PI 값입니다.
static const float RC_PI = 3.14159265f;

/// 사용하지 않는 함수 매개변수를 무시하고 컴파일러 경고를 무음 처리하는 데 사용됩니다.
...
enum rcLogCategory
{
	RC_LOG_PROGRESS = 1,	///< 진행 로그 항목.
	RC_LOG_WARNING,			///< 경고 로그 항목.
	RC_LOG_ERROR			///< 오류 로그 항목.
};

/// 성능 타이머 카테고리를 다시 캐스팅합니다.
/// @see rcContext
enum rcTimerLabel
{
	/// 사용자가 정의한 빌드 총 시간입니다.
	...
	/// 사용자 정의 빌드 시간입니다.
	RC_TIMER_TEMP,
	/// 트라이앵글을 래스터화할 시간입니다. (참조: #rcRasterizeTriangle)
	RC_TIMER_RASTERIZE_TRIANGLES,
	/// 컴팩트 하이트필드를 빌드할 시간입니다. (참조: #rcBuildCompactHeightfield).
	RC_TIMER_BUILD_COMPACTHEIGHTFIELD,
	/// 윤곽선을 만드는 데 걸린 총 시간입니다. (참조: #rcBuildContours)
	RC_TIMER_BUILD_CONTOURS,
	/// 윤곽선의 경계를 추적할 시간입니다. (참조: #rcBuildContours)
	RC_TIMER_BUILD_CONTOURS_TRACE,
	/// 윤곽을 단순화할 시간입니다. (참조: #rcBuildContours)
	RC_TIMER_BUILD_CONTOURS_SIMPLIFY,
	/// 렛지 스팬을 필터링할 시간입니다. (참조: #rcFilterLedgeSpans).
	RC_TIMER_FILTER_BORDER,
	/// 낮은 높이 범위를 필터링할 시간입니다. (참조: #rcFilterWalkableLowHeightSpans)
...
public:
	...
	inline void startTimer(const 
		rcTimerLabel label) { if (m_timerEnabled) doStartTimer(label); }
