...
#include "../KunrealEngine/SceneManager.h"
...
Deserialize::Deserialize()
	: _scene1First(false), _scene2First(false), _scene3First(false), _scene4First(false), _scene5First(false)
{
...
void Deserialize::Initialize(std::string& deserialize)
{
	...
	//executablePath = executablePath.parent_path().parent_path().parent_path().parent_path() / "EpicTool" / "mapData";

	// 파일명 설정
	//std::string filename = deserialize; // 예시 파일명

	// 파일의 절대 경로 생성
	//std::filesystem::path filePath = executablePath / filename;


	CreateData(deserialize);

}

void Deserialize::CreateData(std::string& deserialize)
{
...
void Deserialize::SetChangeSceneData()
{
